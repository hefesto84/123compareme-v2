        ...
        if(showWidget === false){
            ...
            if($('html').attr('lang') === 'en'){
                if ($('span.reservas_tarifas_precio').length){
                    ...
                    var price = parseFloat($('.activa').find('.price').html().substring(0,$('.activa').find('.price').html().indexOf('<'))).toFixed(2);
                }
            } else {
                if ($('span.reservas_tarifas_precio').length){
                    var price = parseFloat($('span.reservas_tarifas_precio').html().replace('.','').replace(',','.')).toFixed(2);
                } else {
                    var price = parseFloat($('.activa').find('.price').html().substring(0,$('.activa').find('.price').html().indexOf('<')).replace('.','').replace(',','.')).toFixed(2);
                }
            }
